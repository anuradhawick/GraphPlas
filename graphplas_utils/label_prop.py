"""
Code obtained from GraphBin https://github.com/Vini2/GraphBin
Original source can be found at https://github.com/ZwEin27/python-labelpropagation
"""
...
class LabelProp():
    ...
    def __init__(self):
        self.logger = logging.getLogger('GraphPlas')
        self.logger.debug('Creating an instance of LabelProp')
        self.initialize_env()
...
################################################################################
    ...
    def run(self, eps, max_iter, show_log=False, clean_result=False):
        ...
        for i in range(max_iter):
            diff = self.iterate()
            logger.debug(f"Iteration {i+1} Difference {diff}")
            if diff < eps:
                break
...
################################################################################
    ...
    def show_detail(self, diff, eps, i, max_iter):
        logger.debug("Total number of vertices:\t\t" + str(self.vertex_size))
        logger.debug("Number of class labels:\t\t" + str(self.label_size))
        logger.debug("Previous number of unlabeled vertices:\t" + str(self.vertex_size - self.labelled_size))
        logger.debug("Previous numebr of labeled vertices:\t" + str(self.labelled_size))
        logger.debug("Value of eps parameter:\t\t" + str(eps))
        logger.debug("Value of max_iteration parameter:\t" + str(max_iter))
        logger.debug("Final values:")
        logger.debug("iter = " + str(i+1) + ", diff = " +  str(diff))
